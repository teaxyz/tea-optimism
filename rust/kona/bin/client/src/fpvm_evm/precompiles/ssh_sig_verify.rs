//! Accelerated `0x0698` SSHSIG signature-verification precompile (Tea).
//!
...
use tea_precompiles::gas::{SSHSIG_VERIFY_ADDRESS, sshsig_required_gas};

/// Runs the FPVM-accelerated `0x0698` SSHSIG verify precompile call.
pub(crate) fn fpvm_ssh_sig_verify<H, O>(
    input: &[u8],
    gas_limit: u64,
    hint_writer: &H,
    oracle_reader: &O,
...
{
    let gas_used = sshsig_required_gas(input);
    ...
    let result_data = kona_proof::block_on(precompile_run! {
        hint_writer,
        oracle_reader,
        &[SSHSIG_VERIFY_ADDRESS.as_slice(), &gas_used.to_be_bytes(), input]
    })
...
mod test {
    ...
    /// Real ed25519 SSHSIG fixture (valid → bytes32(1)), shared with the EL's
    /// `tea-precompiles` unit tests via the crate's testdata.
    const ED25519_INPUT_HEX: &str =
        include_str!("../../../../../../tea-precompiles/src/testdata_sshsig_verify_ed25519.hex");

    #[tokio::test(flavor = "multi_thread")]
    async fn test_accelerated_ssh_sig_verify_matches_native() {
        test_accelerated_precompile(|hint_writer, oracle_reader| {
            let input = hex::decode(ED25519_INPUT_HEX.trim()).expect("valid hex");
            let accelerated =
                fpvm_ssh_sig_verify(&input, u64::MAX, hint_writer, oracle_reader).unwrap();
            let native =
                execute_native_precompile(SSHSIG_VERIFY_ADDRESS, input.clone(), u64::MAX).unwrap();
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_accelerated_ssh_sig_verify_out_of_gas() {
        test_accelerated_precompile(|hint_writer, oracle_reader| {
            let err = fpvm_ssh_sig_verify(&[0u8; 64], 0, hint_writer, oracle_reader).unwrap_err();
