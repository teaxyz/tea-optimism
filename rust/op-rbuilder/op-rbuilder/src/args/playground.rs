...
impl PlaygroundOptions {
    ...
    pub(super) fn new(path: &Path) -> Result<Self> {
        ...
        let trusted_peer = TrustedPeer::from_secret_key(
            ...
            // Convert secp256k1 SecretKey across crate versions via raw bytes.
            &reth::network::config::SecretKey::from_slice(
                &extract_deterministic_p2p_key(path)?.secret_bytes(),
            )?,
        );
