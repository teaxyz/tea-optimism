...
//! a real EVM instance created by `OpEvmFactory`.
//!
//! Tea's precompiles are injected into `OpEvmFactory` via the `tea-precompiles`
//! crate and the patched `alloy-op-evm`. These tests prove that the integration
//! works end-to-end: the factory creates an EVM with the GPG precompile at
//! `0x0696` and the TEA/ETH L1 cost multiplier set on the context.
